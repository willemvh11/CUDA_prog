...
__device__ void rot_elec( float *w, float *vec, const float dt)
{
    float mw = sqrt(w[0]*w[0] + w[1]*w[1] + w[2]*w[2]);
    float omega[3];
    float invmw = 1.0f/mw;
    float invwt = 1.0f/(mw*dt);
    omega[0] = w[0]*invmw;
    omega[1] = w[1]*invmw;
    omega[2] = w[2]*invmw;
    float dot = omega[0]*vec[0] + omega[1]*vec[1] + omega[2]*vec[2];
    ...
    vec[1] = i1[1] + i2[1]*cwt + i3[1]*swt;
    vec[2] = i1[2] + i2[2]*cwt + i3[2]*swt;
    w[0] = i1[0] + i2[0]*swt*invwt + i3[0]*(1.0-cwt)*invwt;
    w[1] = i1[1] + i2[1]*swt*invwt + i3[1]*(1.0-cwt)*invwt;
    w[2] = i1[2] + i2[2]*swt*invwt + i3[2]*(1.0-cwt)*invwt;
}
...
float *sstore, const int a, const float dt, float *wfin)
{
    ...
    rot_elec (wtemp, sloc+(3*glid), dt);
    wloc[3*glid] = wtemp[0];
    wloc[3*glid + 1] = wtemp[1];
    wloc[3*glid + 2] = wtemp[2];
    __syncthreads();
    int wind = 3*nl*groupid + glid;
    for(int ii = 0; ii < 3; ++ii, wind += nl)
    {
...
int main(void)
{
    ...
    */
    ...
    Rzztxt.open("Rzz_w=0_int_tstep=taumul20.txt");
